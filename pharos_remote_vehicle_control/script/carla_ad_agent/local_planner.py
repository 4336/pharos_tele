...
class LocalPlanner(CompatibleNode):
    ...
    def __init__(self):
        ...
        self._odometry_subscriber = self.new_subscription(
            ...
            self.odometry_cb,
            qos_profile=10)
        self._virtual_vehicle_odometry_subscriber = self.new_subscription(
            Odometry,
            "/virtual/virtual_vehicle/odometry",
            self.vv_odometry_cb,
            qos_profile=10)
    ...
    def vv_odometry_cb(self, odometry_msg):
        with self.data_lock:
            self._target_pose = odometry_msg.pose.pose
    ...
    def path_cb(self, path_msg):
        with self.data_lock:
            # print(self.path_cnt)
            self.path_cnt = self.path_cnt + 1
            if self.path_cnt > 30 * 0.5:
                self.path_cnt = 0
